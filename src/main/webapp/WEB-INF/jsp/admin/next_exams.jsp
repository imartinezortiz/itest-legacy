...
<% 
	...
	breadCrumb.addStep("Próximos exámenes programados",request.getContextPath()+"/admin/admin.itest?method=nextExams");
...
<script type="text/javascript">
	...
	function orderTable(sort){
		iTestLockPage();
		AdminMgmt.orderNextExamsList(sort,reverse,{callback:updateTable,
	...
	function findExams(){
		...
		iTestLockPage();
		AdminMgmt.getNextExamsFiltered(center,course,startDate,endDate,{callback:updateTable,
			 timeout:callBackTimeOut,
			 errorHandler:function(message) { iTestUnlockPage('error');} });
	...
	function updateTable(list){
		...
		tbodyelement.setAttribute("id","nextExamsTableBody");
		...
		// Replaces tbody			
		datatable.replaceChild(tbodyelement,document.getElementById("nextExamsTableBody"));
	}
...
<fieldset>
	...
	<form>
     ...
     <table id="Tabla1" class="tablaDatos">
			...
			<col width="0*"/>	
			<tr>
		     	<th colspan="5"><center><fmt:message key="textTableHeadNextExams"/></center></th>
		    </tr>
     		<tr>
	     ...
	     <tbody id="nextExamsTableBody">
		    <c:forEach items="${nextExams}" var="exam">
		     	<tr>
		     		<td>
